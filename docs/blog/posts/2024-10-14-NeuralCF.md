...
embedding layers of both `user` & `items`.

The `NeuMF` model contains both of these parts, which need to be concatenated together and a linear layer is placed as the final output layer.
