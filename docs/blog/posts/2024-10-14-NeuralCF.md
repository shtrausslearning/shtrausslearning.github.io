...
The most important part is probably the neural network itself, lets 
...
So we have a general overiew of how the neural network is created, using PyTorch, we'll put it into realisation. The model is essentially formed using two parts, `embedding` layers and `Sequential` segment.
...
def metrics(model, test_loader, top_k):
	...
	return precision(df),recall(df),mrr(df)
```
