...
We will be using a factor number of 32 dimensions. And 4 negative samples are added per positive sample (maximum). 10 iterations should be enough to evaluate our model accuracy. The other parameters are defined as following:
...
Our labels contain two classes, so we'll use `BCE`. As for the optimiser, `Adam` will be used. A learning rate of 0.001 is used in all models.
...
user, item and labels data that is passed into 5r forward pass is in the format:
