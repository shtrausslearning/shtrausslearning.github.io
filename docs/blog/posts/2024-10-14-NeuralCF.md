...
First we need to define the embedding layers, for the MLP embedding layers we need to make sure they are of the same size as the input of the MLP layers (Sequential)

Most notably here we need the to set the `user_num` (number of users), `item_num` (number of items) and `factor_num` (number of factors).
...
Let's take a look at the layers of the network, after having setup the model using 
