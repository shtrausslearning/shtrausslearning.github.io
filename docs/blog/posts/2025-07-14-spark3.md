...
<div style="width: 100%; font-family: Trebuchet MS; font-weight: bold;">
    <div style="padding-top: 40%; position: relative; background-color: #000000; border-radius:10px;">
        <div style="background-image: url('https://towardsdatascience.com/wp-content/uploads/2024/08/1_koHOjMO3-QZaWL3sluaXg.png'); background-size: cover; background-position: center; position: absolute; top: 0; left: 0; right: 0; bottom: 0; opacity: 0.2; border-radius:10px">
        </div>
...
## **:material-format-list-text: Overview**

Todays post covers the following:

- **Missing data** treatment pipeline
- Feature scaling using **ScandardScaler** pipeline
...
Once we have our **assembler**, we can scal our features
...
### **:material-stack-overflow: OHE Pipline**

**One Hot Encoding** sample pipeline in pyspark
...
# Stage 1: Convert categorical feature to numeric indices
...
predictions.select("id", "category", "label", "prediction", "probability").show()
```
...
+---+--------+-----+----------+--------------------+
|  0|       A|    0|       0.0|[0.72179987059079...|
|  1|       B|    1|       1.0|[0.42008160458452...|
|  2|       A|    0|       0.0|[0.80245062727414...|
|  3|       B|    1|       0.0|[0.53141760657688...|
|  4|       C|    0|       1.0|[0.24048737291084...|
|  5|       C|    1|       1.0|[0.28376293539192...|
+---+--------+-----+----------+--------------------+
```
