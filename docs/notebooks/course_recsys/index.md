...
- <center><b>[Look-a-like Сегментация Пользователей](Look-a-like_Segmentation)</b></center>
	...
	Посмотрим на задачу look-a-like в которой мы используем бинарную классификацию для предсказания подобных пользователей. Посмотрим на примере оттока клиетнов из банка, у нас есть исторические данные о клиенах которые ушли и те которые не ушли. Строим модель которая учится находить похожих клиентов.
